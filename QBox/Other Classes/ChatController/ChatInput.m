...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        if (UIInterfaceOrientationIsPortrait(myOrientation))
        {
            ...
            //self.frame = CGRectMake(0, ScreenHeight(), ScreenWidth(), 40);
        ...
        _textView.frame = CGRectMake(10, 6, self.bounds.size.width - 75, 28);
               _textView.delegate = self;
       // [_textView setContentInset:UIEdgeInsetsMake(-55.0, 0.0, 0.0, 0.0)];
       ...
       // _sendBtnActiveColor = [UIColor colorWithRed:0.142954 green:0.60323 blue:0.862548 alpha:1];
        _sendBtnActiveColor=[UIColor blackColor];
        _sendBtnInactiveColor = [UIColor blackColor];
        ...
        _bgToolbar.backgroundColor=[UIColor clearColor];
...
- (void)sendBtnPressed:(id)sender
{
    ...
    {
        UIAlertView *alertView=[[UIAlertView alloc]initWithTitle:@"Alert!!" message:@"Enter some text" delegate:nil cancelButtonTitle:nil otherButtonTitles:@"Ok", nil];
        [alertView show];
    }
}
...
- (void) resizeView
{
    ...
    if (UIInterfaceOrientationIsLandscape([UIApplication sharedApplication].statusBarOrientation)) {
        ...
        else inputStartingPoint = ScreenHeight()-50;
    }
...
- (void) alignTextViewWithAnimation:(BOOL)shouldAnimate
{
    ...
    CGRect line=CGRectMake(10, 7, 2, 20);
    //CGRect line = [_textView caretRectForPosition:_textView.selectedTextRange.end];
